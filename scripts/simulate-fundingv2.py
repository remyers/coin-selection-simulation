...
def next_change_target(utxo_targets, target_counts, bucket_utxos_count, feerates_satoshis_per_kvbyte, tx_size_vbytes):
    min_bucket = -1
    min_capacity = None
    ...
    for i in range(len(utxo_targets)):
        capacity = float(target_counts[i]) / (bucket_utxos_count*utxo_targets[i]['target_utxo_percent'])
...
def split_to_change_targets(change_amount, change_target, utxo_targets, target_counts, bucket_utxos_count, feerates_satoshis_per_kvbyte, change_fee_sat, max_outputs):
    ...
    while (max_outputs < 0 or len(change_outputs) < max_outputs):
        ...
        bucket, capacity, target_amount = next_change_target(utxo_targets, target_counts, bucket_utxos_count, feerates_satoshis_per_kvbyte, tx_size_vbytes)
        if (capacity >= 1.0):
            break
...
for bin in feerates_satoshis_per_kvbyte:
    total_feerate_probability += bin['probability']
assert(round(total_feerate_probability,6) == 1.0)
accum_prob = 0

bucket_utxos_count = json_dec['bucket_utxos_count']
bucket_refill_probability = json_dec['bucket_refill_probability']

# compute bucket_refille_feerate from bucket_refill_probability
for bin in feerates_satoshis_per_kvbyte:
    diff = accum_prob + bin['probability'] - bucket_refill_probability
    if diff > 0:
        bucket_refill_feerate = bin['start'] + (bin['end'] - bin['start'])*((bin['probability']-diff)/bin['probability'])
        break
...
for _ in range(int(args.repeat)):
    with open(args.scenario) as scenario_file:
                        ...
                        if excess > change_fee:
                            ...
                            if amount >= amount_needed:
                                ...
                                if to_sat(change_amount) > to_sat(change_fee) + utxo_targets[0]['start_satoshis'] + min_fee:
                                    _, _, change_target = next_change_target(utxo_targets, target_counts, bucket_utxos_count, feerates_satoshis_per_kvbyte, tx_size_vbytes)
                                    if refill and to_sat(change_amount) > to_sat(change_fee) + change_target:
                                        # split change to refill as many buckets as possible
                                        outputs_sats = split_to_change_targets(to_sat(change_amount), change_target, utxo_targets, target_counts, bucket_utxos_count, feerates_satoshis_per_kvbyte, to_sat(change_fee), -1)
                                    else:
        ...
        results_stats[-1]["bucket_utxos_count"]=bucket_utxos_count
        results_stats[-1]["bucket_refill_probability"]=bucket_refill_probability
        results_stats[-1]["bucket_refill_feerate"]=bucket_refill_feerate
