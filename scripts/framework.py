...
class Simulation:
    ...
    def parse_args(self):
        ...
        parser.add_argument(
            ...
            dest="extra_configs",
            default="",
            help="Comma seperated list of extra configuration lines to include in bitcoin.conf"
        )
        parser.add_argument(
            "--delay_confirmation",
            dest="delay_confirmation",
            type=int,
            default=0,
            help="Delay confirmation of transactions for this many blocks after it is created (default: 0)."
        )
        parser.add_argument(
            "--payment_amount",
            dest="payment_amount",
        ...
        parser.add_argument(
            ...
            dest="utxo_targets",
            default="",
            help="JSON file with target utxo amounts and counts (default: <none>)."
        )
        parser.add_argument(
            "--disable_algos",
            dest="disable_algos",
            default=False,
            action="store_true",
            help="Disable Knapsack and SRD coin selection algorithms.",
        )
        parser.add_argument(
            "--excess_percent",
            dest="excess_percent",
            type=float,
            default=0.05,
            help="when no change, adjust target amount by up to this percent, default: 0.05 (ie. +/- 5%)"
        )
    ...
    def setup_nodes(self):
        self.add_nodes(self.num_nodes, self.extra_args, self.options.extra_configs.split(","))
        self.start_nodes()

    def add_nodes(self, num_nodes: int, extra_args=None, extra_configs=None):
        extra_confs = [["bind=127.0.0.1"]+extra_configs] * num_nodes
