...
def split_residual_change(change_outputs, utxo_targets, target_counts, change_fee, min_fee, max_fee, bucket_max):
    ...
    if change_outputs[-1] <= bucket_max and bucket == -1:
        ...
        for _ in range(20):
            ...
            numbers.append(target_amount + change_fee)
        ...
        for output_amount in best_combination:
            change_outputs.append(output_amount - change_fee)

    return change_outputs

# split change output to replentish utxo_targets
def split_to_change_targets(change_amount, change_target, utxo_targets, target_counts, feerates_satoshis_per_kvbyte, change_fee):
...
def compute_target_counts(utxos, pending_utxos, utxo_targets, feerates_satoshis_per_kvbyte, tx_size_vbytes):
    ...
    for pending_tx in pending_utxos:
...
with open(args.scenario) as scenario_file:
    ...
    for row in scenarioreader:
        ...
        tx_fee = satoshi_round(feerate * (tx_size_vbytes/1000)) # 1 input, 1 output
        change_fee = to_coin(int(Decimal(COIN * Decimal(feerate) * Decimal(43) / Decimal(1000.0)).quantize(1, ROUND_UP)))
        if spend < 0:
        ...
        target_counts = compute_target_counts(utxos, pending_utxos, utxo_targets, feerates_satoshis_per_kvbyte, tx_size_vbytes)
        ...
        if spend > 0:
            ...
            for amount in sorted_utxos:
                ...
                if satoshi_round(amount) > amount_needed:
                    ...
                    if total_fees > spend*Decimal(0.05):
                        ...
                        if amount >= amount_needed:
                            ...
                            if feerate < to_coin(bucket_refill_feerate):
                                outputs_sats = split_to_change_targets(to_sat(change_amount), change_target, utxo_targets, target_counts, feerates_satoshis_per_kvbyte, to_sat(change_fee))
                            else:
            ...
            if len(pending_utxos) >= 50:
                confirmed_utxos = pending_utxos.pop(0)
                for utxo in confirmed_utxos:
                    utxos.append(utxo)

    total_pending_utxo_value = 0
    for tx in pending_utxos:
        for utxo in tx:
    ...
    new_f.write(f"target_file={args.targets}\n")
    new_f.write(f"bucket_refill_feerate={to_coin(bucket_refill_feerate)}\n")
    new_f.write(f"minimum_total_fees={min_total_fees}\n")
    new_f.write(f"simulated_total_fees={sim_total_fees}\n")
    new_f.write(f"simulated_total_excess={sim_total_excess}\n")
    new_f.write(f"utxos_count={len(utxos)}\n")
    new_f.write(f"target_counts={target_counts}\n")
    new_f.write(f"total_sent_value={total_sent_value}\n")
    new_f.write(f"total_utxo_value={sum(utxos)}\n")
    new_f.write(f"total_pending_utxo_value={total_pending_utxo_value}\n")
    new_f.write(f"total_utxo_value+total_sent_value+total_fees+total_pending_utxo_value={sum(utxos)+total_sent_value+sim_total_fees+total_pending_utxo_value}\n")
    new_f.write(f"total_funding_value={total_funding_value}\n")
    new_f.write(f"received_utxos={len(received_utxos)}\n")
    new_f.write(f"received_utxos_value={sum(received_utxos)}\n")
    new_f.write(f"total_changeless={len(changeless_txs)}\n")
    new_f.write(f"total_with_change={len(with_change_txs)}\n")
    new_f.write(f"utxos_not_in_buckets={len(utxos_not_in_buckets)}\n")
    new_f.write(f"utxos_in_buckets={len(utxos_in_buckets)}\n")
    new_f.write(f"small_change_outputs_not_in_buckets={len(small_change_outputs_not_in_buckets)}\n")
    new_f.write(f"small_utxos_not_in_buckets={len(small_utxos_not_in_bucket)}\n")
    new_f.write(f"failures: {len(failures)}\n")
    for failure in failures:
        new_f.write(f"\t{failure}\n")
    new_f.write("\"feerates_satoshis_per_kvbyte\":\n")
    feerates = dict(sorted(feerates.items()))

    feereates_list = []
    current_feerate = {}
    current_feerate = {'start': 1000, 'end': 0, 'probability': 0}
    current_probability = 0
    for bin in feerates:
         current_probability += feerates[bin]/count
         if (current_probability >= 0.05):
            current_feerate['end'] = bin*1000-1
            ...
            current_feerate = {'start': bin*1000, 'end': 0, 'probability': current_probability}
    current_feerate['end'] = bin*1000-1
    current_feerate['probability'] = round(current_probability, 2)
    feereates_list.append(current_feerate)
